...
public class GuiCreateCustomWorld extends GuiCreateWorld {
		...
		try {
			...
			for (int i = 0; i < WorldType.worldTypes.length; i++) {
				if (WorldType.worldTypes[i] != null
					...
					if (WorldType.worldTypes[i]
							.getWorldTypeName()
							.equalsIgnoreCase(
									ConfigGeneralSettings.generalWorldGenerator)) {
						WorldGenerator = WorldType.worldTypes[i]
								.getWorldTypeID();
						Log.info("Changed world type to "
								+ WorldType.worldTypes[i].getTranslateName());
					}
				}
			}
	...
	public void initGui() {
		...
		try {
			...
			guiReflectHelper.func_146319_h.invoke(this);
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		try {
			if (!guiReflectHelper.field_146344_y.getBoolean(this)) {
				super.drawScreen(mouseX, mouseY, partialTicks);
				return;
			}
			...
			if (((GuiButton) guiReflectHelper.btnMapFeatures.get(this)).visible) {
				this.drawString(this.fontRendererObj, I18n.format(
						"selectWorld.mapFeatures.info", new Object[0]),
			...
			((GuiTextField) guiReflectHelper.field_146335_h.get(this))
					.drawTextBox();

			for (int i = 0; i < this.buttonList.size(); ++i) {
				((GuiButton) this.buttonList.get(i)).drawButton(this.mc,
						mouseX, mouseY);
			}

			for (int j = 0; j < this.labelList.size(); ++j) {
				((GuiLabel) this.labelList.get(j)).drawLabel(this.mc, mouseX,
						mouseY);
			}
		} catch (Exception ex) {
