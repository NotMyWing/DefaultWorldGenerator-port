...
public class guiReflectHelper {
	...
	public static void initReflect(){
		/* determine if we are in dev or not, then load the private fields/methods
		 * we will access via reflection.
		 */
		boolean deob;
		try{
		...
		try{
			if(deob){
				...
				field_146335_h = GuiCreateWorld.class.getDeclaredField("field_146335_h");
				field_146344_y = GuiCreateWorld.class.getDeclaredField("field_146344_y");
				
				func_146319_h = GuiCreateWorld.class.getDeclaredMethod("func_146319_h");
			} else {
